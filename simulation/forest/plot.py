...
def make_plot(ax, burning_list, burned_list, forest, mode):
    """Make a plot of a forest fire.

    Arguments:
        ax {Axes} -- Axes to plot on.
        burning_list {ndarray} -- Burning trees.
        burned_list {ndarray} -- Burned trees.
...
def plot_forest(burning_list, burned_list, ax, mode=lattice.LATTICE_SQUARE):
    ...
    Keyword Arguments:
        mode {int} -- Lattice type. (default: {lattice.LATTICE_SQUARE})
    """
...
def plot_bonds(burning_list, burned_list, forest, ax,
    ...
    """Plot bonds connecting lattice sites.
